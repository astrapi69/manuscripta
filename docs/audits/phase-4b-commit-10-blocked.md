...
The triage map was formally reconstructed from a fresh mutmut run
(per-mutant diffs via `mutmut show`) and approved:
[phase-4b-commit-10-triage-map.md](phase-4b-commit-10-triage-map.md).
Commit 10 landed against it. Final module state: 99.7 % (1 C-category
survivor, 23 annotated equivalents, 0 timeouts). This document is
retained as the audit record of the block; the reconstruction task it
